...
HELP_TOPICS = {
    "О системе": (
        "«Диспетчерская служба аэропорта» — настольное приложение для ведения "
        "справочников, управления расписанием рейсов и подготовки отчетов.\n\n"
        "Разработчик: VladimirO3.\n"
        "Технологии: Python, Tkinter/ttk и SQLite.\n"
        "Исходный код проекта: github.com/VladimirO3/AirportMs."
    ),
    "Обзор системы": (
    ...
    "Пассажиры": (
        "На вкладке «Пассажиры» ведётся список пассажиров. Укажите ФИО и "
        "уникальный номер паспорта; телефон и e-mail являются необязательными. "
        "Выберите строку для изменения или удаления. Пассажира, связанного с "
        "билетом, удалить нельзя."
    ),
    "Сотрудники": (
        "На вкладке «Сотрудники» ведётся список сотрудников аэропорта. "
        "Укажите ФИО и должность, а телефон можно оставить пустым. "
        "Записи поддерживают добавление, изменение и удаление."
    ),
    "Билеты": (
        "На вкладке «Билеты» создаются и редактируются билеты. Выберите "
        "существующего пассажира и рейс, укажите уникальный номер билета, "
        "место, статус и положительную цену."
    ),
    "Проверка данных": (
    ...
    "Отчеты и печать": (
        ...
        "расписание рейсов, списки авиакомпаний, аэропортов, самолётов, "
        "пассажиров, сотрудников и билетов, а также сводка рейсов по статусам.\n\n"
